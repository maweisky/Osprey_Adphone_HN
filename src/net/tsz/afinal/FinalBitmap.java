...
//	}
	...
	public void display(View imageView,String uri,int imageWidth,int imageHeight,boolean isRound){
	...
	public void display(View imageView,String uri,Bitmap loadingBitmap,Bitmap laodfailBitmap,boolean isRound){
	...
	public void display(View imageView,String uri,int imageWidth,int imageHeight,Bitmap loadingBitmap,Bitmap laodfailBitmap,boolean isRound){
	...
	private void doDisplay(View imageView, String uri, BitmapDisplayConfig displayConfig,boolean isRound) {
		if(!mInit ){
			init();
		}
...
//			}
	...
	private class BitmapLoadAndDisplayTask extends AsyncTask<Object, Void, Bitmap> {
		...
		public BitmapLoadAndDisplayTask(LoadBitmapCallback callback,BitmapDisplayConfig config) {
		...
		protected void onPostExecute(Bitmap bitmap) {
			...
			if(imageViewReference!=null){
				...
				if (bitmap != null && imageView != null) {
					mConfig.displayer.loadCompletedisplay(imageView,bitmap,displayConfig, mFinalBitmap,isRound);			
				}else if(bitmap == null && imageView!=null ){
					mConfig.displayer.loadFailDisplay(imageView, displayConfig.getLoadfailBitmap(),mFinalBitmap,isRound);
				}
			}else{
	...
	public void viewSetImage(View view, Bitmap bitmap,BitmapDisplayConfig displayConfig,boolean isRound) {
		...
		if(isRound){
			if (view instanceof ImageView) {
				...
				Drawable borderedRoundedAvatarDrawable = avatarDrawableFactory
						.getBorderedRoundedAvatarDrawable(displayConfig,bitmap);
				((ImageView) view).setImageDrawable(borderedRoundedAvatarDrawable);
			} else {
				AvatarDrawableFactory avatarDrawableFactory = new AvatarDrawableFactory(
						mContext.getResources());
				Drawable borderedRoundedAvatarDrawable = avatarDrawableFactory
						.getBorderedRoundedAvatarDrawable(displayConfig,bitmap);
				view.setBackgroundDrawable(borderedRoundedAvatarDrawable);
			}
		}else{
			if (view instanceof ImageView) {
	...
	public void loadBmpWithConfig(String uri,int imageWidth,int imageHeight, LoadBitmapCallback callback){
		BitmapDisplayConfig displayConfig = configMap.get(imageWidth+"_"+imageHeight);
		if(displayConfig==null){
			displayConfig = getDisplayConfig();
			displayConfig.setBitmapHeight(imageHeight);
	 ...
	 *            当图片加载成功后调用接口方法
	 */
	public void loadBmpWithConfig(String imageUrl, BitmapDisplayConfig displayConfig,
	...
	public void loadBmpWithWH(String uri, BitmapDisplayConfig displayConfig,
		...
		if (bitmap != null) {
			...
			final BitmapLoadAndDisplayTask task = new BitmapLoadAndDisplayTask(callback,displayConfig);
