...
public class GrzxJfscGridAdapter extends BaseAdapter {
	...
	public GrzxJfscGridAdapter(Context context, List<JfscSpList> list) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = inflater.inflate(
					R.layout.os_dhb_grzx_jfsc_gridview_item, null);
			...
			viewHolder.tv_jf_num = (MarqueeText) convertView
		...
		if (infos.get(position).getTitle().length() > 20) {
		...
		if (infos.get(position).getSimpleinfo().length() > 8) {
		...
		span.setSpan(
				new ForegroundColorSpan(mContext.getResources().getColor(
						R.color.deep_red)), 5, viewHolder.tv_jf_num.getText()
	...
	static class ViewHolder {
		...
		private TextView tv_name, tv_sp_num;
		private MarqueeText tv_describe, tv_jf_num;
	}
